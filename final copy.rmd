...
# OLS models for Gini coefficients using lm() for consistency
ols_gini_w <- lm(lngini_w ~ dism1990, data = df)
ols_gini_b <- lm(lngini_b ~ dism1990, data = df)
...
# OLS models for poverty rates using lm() for consistency
ols_pov_w <- lm(povrate_w ~ dism1990, data = df)
ols_pov_b <- lm(povrate_b ~ dism1990, data = df)

# IV models for poverty rates
...
ols_gini_w_rse <- sqrt(diag(vcovHC(ols_gini_w, type = "HC1")))
ols_gini_b_rse <- sqrt(diag(vcovHC(ols_gini_b, type = "HC1")))
ols_pov_w_rse <- sqrt(diag(vcovHC(ols_pov_w, type = "HC1")))
ols_pov_b_rse <- sqrt(diag(vcovHC(ols_pov_b, type = "HC1")))
...
# Generate table for Gini coefficients (within-race inequality)
stargazer(ols_gini_w, iv_gini_w, ols_gini_b, iv_gini_b,
          se = list(ols_gini_w_rse, iv_gini_w_rse,
                    ols_gini_b_rse, iv_gini_b_rse),
          type = "latex",
          title = "Table 2, Panel 1A: Effects of Segregation on Within-Race Inequality (Gini Coefficients)",
...
# Generate table for poverty rates
stargazer(ols_pov_w, iv_pov_w, ols_pov_b, iv_pov_b,
          se = list(ols_pov_w_rse, iv_pov_w_rse,
                    ols_pov_b_rse, iv_pov_b_rse),
          type = "latex",
          title = "Table 2, Panel 1B: Effects of Segregation on Poverty Rates",
          column.labels = c("OLS", "2SLS", "OLS", "2SLS"),
          dep.var.labels = c("Poverty (White)", "Poverty (White)",
...
# OLS models for between-race inequality measures using lm() for consistency
ols_90w90b <- lm(ln90w90b ~ dism1990, data = df)
ols_10w10b <- lm(ln10w10b ~ dism1990, data = df)
ols_90w10b <- lm(ln90w10b ~ dism1990, data = df)
ols_90b10w <- lm(ln90b10w ~ dism1990, data = df)
...
# Calculate robust standard errors for all models
ols_90w90b_rse <- sqrt(diag(vcovHC(ols_90w90b, type = "HC1")))
ols_10w10b_rse <- sqrt(diag(vcovHC(ols_10w10b, type = "HC1")))
ols_90w10b_rse <- sqrt(diag(vcovHC(ols_90w10b, type = "HC1")))
ols_90b10w_rse <- sqrt(diag(vcovHC(ols_90b10w, type = "HC1")))
...
# Generate Table 2, Panel 2A: Same-percentile comparisons
stargazer(ols_90w90b, iv_90w90b, ols_10w10b, iv_10w10b,
          se = list(ols_90w90b_rse, iv_90w90b_rse,
                    ols_10w10b_rse, iv_10w10b_rse),
          type = "latex",
          title = "Table 2, Panel 2A: Between-Race Inequality at Same Percentiles",
          column.labels = c("OLS", "2SLS", "OLS", "2SLS"),
...
# Generate Table 2, Panel 2B: Cross-distribution comparisons
stargazer(ols_90w10b, iv_90w10b, ols_90b10w, iv_90b10w,
          se = list(ols_90w10b_rse, iv_90w10b_rse,
                    ols_90b10w_rse, iv_90b10w_rse),
          type = "latex",
          title = "Table 2, Panel 2B: Between-Race Inequality Cross-Distribution",
          column.labels = c("OLS", "2SLS", "OLS", "2SLS"),
          dep.var.labels = c("ln(90W/10B)", "ln(90W/10B)",
